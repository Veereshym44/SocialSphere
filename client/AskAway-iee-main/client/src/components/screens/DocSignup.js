...
const handleInputs=(e)=>{
  ...
  const PostData=async(e)=>{
    e.preventDefault();
    const{ name,email,password,docID}=user;
    const res=await fetch("/doc-signup",{
      method:"POST",
      headers:{
        "Content-Type":"application/json"
      },
      body:JSON.stringify({
        name,email,password,docID
      })
    });
  ...
  return (
    <div className='mycard'>
      <div className='card auth-card'>
        ...
        <input type="text" className='input-field' placeholder='Doctor ID' name='docID' value={user.docID} onChange={handleInputs} autoComplete="off"/>
